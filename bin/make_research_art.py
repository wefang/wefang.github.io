...
# the alignment is broadly collinear, with drift in spacing, one local inversion (E2 and E3 swapped in
# zebrafish) and one moved element (E4 sits before E3 in mouse). Z is a
...
POS = {
    "zebrafish": {"E1": 0.3, "E3": 0.44, "E2": 0.6, "Z": 0.77},
    "mouse":     {"E1": 0.22, "E2": 0.4, "E4": 0.55, "E3": 0.68},
    "macaque":   {"E1": 0.26, "P": 0.35, "E2": 0.47, "E3": 0.62, "E4": 0.76},
    "human":     {"E1": 0.24, "P": 0.34, "E2": 0.45, "E3": 0.6, "E4": 0.77},
}
GA = {"zebrafish": 0.06, "mouse": 0.02, "macaque": 0.04, "human": 0.04}      # gene A, left flank
GB = {"zebrafish": 0.84, "mouse": 0.89, "macaque": 0.87, "human": 0.88}      # gene B, reverse strand
...
def path(kind, t, key):
    bend, tilt = WARP[key]
    if kind == "trunk":                                   # blue, left to the fork
        return 0.05 + (FORK[0] - 0.05) * t, 0.46 + (FORK[1] - 0.46) * t + bend * np.sin(np.pi * t)
    if kind == "up":                                      # green, up and right from the fork
...
def spread(a, b, sa, sb):
    return np.clip(a + rs.normal(0, sa, np.shape(a)), 0.03, 0.97), np.clip(b + rs.normal(0, sb, np.shape(b)), 0.05, 0.95)


SEGS = [(1, "trunk", 0.0, 1.0, 60), (0, "up", 0.08, 1.0, 55), (2, "down", 0.08, 1.0, 55), (3, "late", 0.15, 1.0, 24)]
CENT = {}
for k, (lab, key) in enumerate(SPECIES):
    draw_plane(Rk, k, key, 9 - k)
    for j, kind, t0, t1, n in SEGS:
        cen = Rk(k, *path(kind, 0.6 if kind != "trunk" else 0.5, key))
        CENT[(key, j)] = cen
        if j in MISSING[key]:
            ax.add_patch(Ellipse(cen, 0.5 if j == 2 else 0.36, 0.18, fc="none", ec=SCOL[j], lw=1.2,
                                 ls=(0, (1.5, 1.5)), zorder=12))
            continue
        t = rs.uniform(t0, t1, n)
